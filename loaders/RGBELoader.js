...
// simplifly from three's RGBELoader

// https://github.com/mrdoob/three.js/issues/5552
// http://en.wikipedia.org/wiki/RGBE_image_format

const HalfFloatType = 0;
const FloatType = 1;
...
class RGBELoader {
   ...
   * @returns
   */
  static parse(buffer, textureDataType) {
    ...
    if (RGBE_RETURN_FAILURE !== rgbe_header_info) {
      ...
      if (RGBE_RETURN_FAILURE !== image_rgba_data) {
        ...
        switch (textureDataType) {
          ...
          default:
            console.error('THREE.RGBELoader: unsupported type: ', textureDataType);
            break;
        }
  ...
  static async load(url, type = HalfFloatType) {
    const buffer = await (await fetch(url)).arrayBuffer();
    const texture = this.parse(buffer, type);
    return texture;
  }
}
